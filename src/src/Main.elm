    ...
    let
        (newState, cmd) =
            case (model.state, msg) of
                ...
                -- Cazul când primim schimbari de configurare
                ( Model.LoadedPosts state, ConfigChanged change ) ->
                    let
                        -- Aplicare schimbari pe configuratia curenta
                        newConfig = Model.PostsConfig.applyChanges change state.config
                    in
                    ( Model.LoadedPosts { state | config = newConfig }
                    , Effect.NoEffect
                    )

                -- Alte cazuri, daca nu este nicio schimbare de configurare
                ( state, _ ) ->
